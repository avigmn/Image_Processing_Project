...
def compute_mi_scores(feat_class1, feat_class2, num_bins=100):
    """
    Compute mutual information score and best threshold per feature.
    Returns: (indices of top features sorted by MI, MI scores, thresholds)
    """
...
if __name__ == "__main__":
    print("Loading DCT features for DCT + SVM training...")
    ...
    # Build feature matrix using raw (signed) DCT values
    X_waving  = waving_sample[:, top_indices]
    X_walking = walking_sample[:, top_indices]
    ...
    print("Training SVM (RBF kernel, C=1)...")
    svm = SVC(kernel='rbf', C=1.0, gamma='scale', probability=False)
    svm.fit(X_scaled, y)
    print("Training complete.")

    model = {'svm': svm, 'scaler': scaler, 'indices': top_indices}
    model_path = os.path.join(data_dir, "dct_svm_model.joblib")
