...
if __name__ == "__main__":
    ...
    waving_files = [f for f in glob.glob(os.path.join(data_dir, "waving_*_original_features.npy")) if "test" not in f.lower()]
    waving_features = np.vstack([np.load(f) for f in waving_files])
    
    walking_files = [f for f in glob.glob(os.path.join(data_dir, "walking_*_original_features.npy")) if "test" not in f.lower()]
